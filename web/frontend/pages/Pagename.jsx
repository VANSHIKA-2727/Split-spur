...
export default function SplitspurLanding() {
  ...
  // --- CALCULATE SUMMARY STATS AUTOMATICALLY ---
  const totalActive = activeTests.filter(t => t.status === "Test is running" || t.status === "Active").length;
  const totalCompleted = activeTests.filter(t => t.status === "Completed").length;
  // Placeholder for AI wins logic
  const aiWins = activeTests.filter(t => t.confidence > 90).length;
  ...
  const handleConfirmPause = () => {
    console.log('Pausing test:', selectedTest.testName);
    setShowPauseModal(false);
    setSelectedTest(null);
  };

  // Determine which tests to show based on state
  const displayedTests = isViewAll ? activeTests : activeTests.slice(0, 3);
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            ...
            <p className="text-green-600 text-sm">Historical total</p>
          </div>

          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-gray-600 text-sm mb-1">High Confidence Wins</p>
                <p className="text-3xl font-bold">{aiWins}</p>
              </div>
              <div className="w-10 h-10 bg-purple-500 rounded flex items-center justify-center">
                <Zap className="w-5 h-5 text-white" />
              </div>
            </div>
            <p className="text-green-600 text-sm">90%+ Significance</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-8">
          ...
          <div className="p-6 space-y-4">
            {isLoading ? (
               <p className="text-center text-gray-500 py-4">Loading your tests...</p>
            ) : activeTests.length === 0 ? (
               <div className="text-center py-8">
                  <p className="text-gray-500 mb-4">No tests found in your database.</p>
                  <button onClick={() => navigate('/Testcreatepage')} className="text-blue-600 font-bold">Create your first test →</button>
               </div>
            ) : (
              displayedTests.map((test, index) => (
                <div key={test.id || index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-medium text-gray-900 mb-2">{test.testName}</h3>
                      <span className="inline-flex items-center gap-1 text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded">
                        <div className={`w-2 h-2 rounded-full animate-pulse ${test.status === 'Completed' ? 'bg-gray-400' : 'bg-green-500'}`}></div>
                        {test.status}
                      </span>
                    </div>
                  ...
                  <p className="text-sm text-gray-600 mb-2">
                    Visitors: <span className="font-medium text-gray-900">{test.visitors.toLocaleString()}</span>
                  </p>
          ...
          <div className="bg-white rounded-xl max-w-2xl w-full p-8 relative shadow-xl">
            {selectedTest.visitors >= 1000 ? (
              <>
                ...
                <p className="text-gray-900 mb-6">Pausing {selectedTest.testName}</p>
                <ul className="space-y-2 mb-6">
                  <li className="text-gray-900">
                    • Visitors: <span className="font-semibold text-red-600">{selectedTest.visitors}</span>
                  </li>
                  <li className="text-gray-900">
                    • Current lift: {selectedTest.expectedLift}
                  </li>
                  <li className="text-gray-900">
